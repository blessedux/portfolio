...
function setupHistory() {
    ...
    if (State.hash == "/?skipTest") {
        skipFPSCheck = true;
        url = '/my-work/codeology/blessedux';
    }
    
    // Check if we have an originalPath query parameter (from middleware rewrite)
    var urlParams = new URLSearchParams(window.location.search);
    var originalPath = urlParams.get('originalPath');
    
    if (originalPath) {
        // Use the original path from the middleware
        console.log('Using original path from middleware:', originalPath);
        var pathParts = originalPath.split('/').filter(part => part.length > 0);
        
        if (pathParts.length >= 2) { // if blessedux project
            preload('blessedux', pathParts[1]);
        } else if (pathParts.length === 1) { // if author
            preload(pathParts[0]);
            if (MOBILE_VERSION)
                selected = 0;
        } else {
        ...
        // Find the index of 'codeology' to determine the location part
        for (var i = 0; i < urlArr.length; i++) {
            if (urlArr[i] === 'codeology') {
                locationIndex = i + 1;
                break;
            }
        }
        
        if (locationIndex >= 0 && urlArr[locationIndex + 2]) { // if blessedux project
            preload('blessedux', urlArr[locationIndex + 2]);
        } else if (locationIndex >= 0 && urlArr[locationIndex + 1]) { // if project
            preload(urlArr[locationIndex], urlArr[locationIndex + 1]);
        } else if (locationIndex >= 0 && urlArr[locationIndex] && urlArr[locationIndex] !== 'blessedux') { // if author
            preload(urlArr[locationIndex]);
            if (MOBILE_VERSION)
                selected = 0;
        } else {
            preload('blessedux');
...
function screenshotCompose() {
    ...
    // Handle the new path structure where currentLocation is just the location part (e.g., "blessedux" or "blessedux/project")
...
function resizePostprocessing() {
    ...
    asciiShader.material = new THREE.ShaderMaterial({
        uniforms: {
            ...
            hashTexture: {type: 't', value: THREE.ImageUtils.loadTexture("/my-work/codeology/dist/images/fontHash.png")}, //
...
function updateInfo(main, id, staticid) {
    ...
    if (selected != -1 && main) {
        if (featured)
            updateLocation("blessedux/" + db.project.substr(0, 50));
        else
...
function trackLocation(location){
    ...
    if (HISTORY_ENABLED)
        History.pushState({
            state: 1,
            viewing: location
        }, "Codeology/" + location, "/my-work/codeology/" + location);
}
